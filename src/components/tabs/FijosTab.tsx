  ...
  return (
    <>
      ...
      {fixedExpenses.map((e) => (
        ...
        >
          <div className="w-10 h-10 rounded-[11px] bg-linear-to-br from-yellow-100 to-yellow-200 flex items-center justify-center text-[19px] shrink-0">
            {fixedIcon(e.name)}
          </div>
          ...
          <div className="text-[15px] font-bold text-amber-600 whitespace-nowrap">
            {fmt(e.amount)}
          </div>
          <button
            className="bg-transparent border-none text-[#c7d2fe] text-[20px] cursor-pointer p-1 shrink-0"
            onClick={() => onEdit(e)}
          >
            ✎
          </button>
      ...
      <button
        className="bg-transparent border-2 border-dashed border-[#c7d2fe] rounded-[14px] p-3.25 text-indigo-400 text-[14px] font-semibold cursor-pointer text-center w-full active:bg-indigo-50"
        onClick={onAdd}
      >
        + Añadir gasto fijo
      </button>
