    ...
    >
      <div className="bg-white rounded-[28px_28px_0_0] w-full max-w-120 mx-auto p-[20px_20px_44px] max-h-[92vh] overflow-y-auto">
        ...
        <div className="text-[21px] font-extrabold text-[#1e1b4b] mb-4.5">
          {initial ? "✏️ Editar gasto" : "+ Añadir gasto"}
        </div>
          ...
          <div className="grid grid-cols-4 gap-1.75">
            {CATS.map((c) => (
              <button
                key={c.id}
                onClick={() => setCat(c.id)}
                className={`border-2 rounded-[13px] p-[9px_4px] flex flex-col items-center gap-0.75 cursor-pointer ${cat === c.id ? "border-indigo-600 bg-indigo-50" : "border-slate-200 bg-white"}`}
              >
                <span className="text-[20px]">{c.emoji}</span>
                <span
                  className={`text-[10px] font-semibold text-center ${cat === c.id ? "text-indigo-600" : "text-slate-500"}`}
                >
                  {c.label}
                </span>
              </button>
