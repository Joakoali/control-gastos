...
export default function FixedModal({ expense, onClose, onSave, onDelete }: Props) {
  ...
  return (
    <div className="fixed inset-0 bg-[rgba(15,10,40,0.6)] z-50 flex items-end backdrop-blur-[4px]" onClick={e => e.target === e.currentTarget && onClose()}>
      <div className="bg-white rounded-[28px_28px_0_0] w-full max-w-[480px] mx-auto p-[20px_20px_44px] max-h-[92vh] overflow-y-auto">
        ...
        <div className="text-[21px] font-extrabold text-[#1e1b4b] mb-[18px]">{isNew ? '+ Gasto fijo' : '✏️ Editar fijo'}</div>

        <div className="mb-[14px]">
          <label className="text-[12px] font-bold text-slate-500 uppercase tracking-[0.4px] mb-[6px] block">Nombre</label>
          <input className="w-full p-[13px_14px] border-2 border-slate-200 rounded-[13px] text-[16px] text-[#1e1b4b] outline-none focus:border-indigo-600 bg-white" value={name} onChange={e => setName(e.target.value)} placeholder="Ej: Netflix" autoFocus />
        </div>
        ...
        <div className="flex gap-[10px] mt-5">
          {!isNew && (
            <button className="flex-1 py-[15px] border-none rounded-[13px] bg-red-100 text-[15px] font-bold text-red-500 cursor-pointer" onClick={() => { onDelete(expense!.id); onClose() }}>Eliminar</button>
          )}
