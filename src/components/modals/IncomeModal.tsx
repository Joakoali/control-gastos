...
export default function IncomeModal({ sources, prevSources = [], onClose, onSave }: Props) {
  ...
  const update = (id: string | number, field: keyof IncomeSource, val: string) =>
    setItems(prev => prev.map(i => i.id === id ? { ...i, [field]: val } : i))

  const remove = (id: string | number) => setItems(prev => prev.filter(i => i.id !== id))

  const add = () => setItems(prev => [...prev, { id: newId(), name: 'Extra / Ingreso', amount: '' }])

  const save = () => {
    const cleaned = items
      .map(i => ({ ...i, amount: toFloat(i.amount), name: (i.name as string).trim() || 'Ingreso' }))
  ...
  return (
    <div className="fixed inset-0 bg-[rgba(15,10,40,0.6)] z-50 flex items-end backdrop-blur-[4px]" onClick={e => e.target === e.currentTarget && onClose()}>
      <div className="bg-white rounded-[28px_28px_0_0] w-full max-w-[480px] mx-auto p-[20px_20px_44px] max-h-[92vh] overflow-y-auto">
        ...
        <div className="text-[21px] font-extrabold text-[#1e1b4b] mb-[18px]">💰 Ingresos del mes</div>
        <p className="text-[13px] text-slate-400 mb-3">Añadí sueldos, extras, comisiones o cualquier ingreso de este mes.</p>

        {prevSources.length > 0 && (
          <button onClick={copyFromPrev}
            className={`w-full py-[10px] px-[14px] mb-[14px] border-2 border-dashed border-violet-300 rounded-[12px] text-[14px] font-semibold cursor-pointer flex items-center justify-center gap-2 transition-all ${copied ? 'bg-green-50 text-emerald-600' : 'bg-violet-50 text-violet-700'}`}>
            {copied ? '✅ Copiado!' : '📋 Copiar sueldos del mes anterior'}
          </button>
        )}

        {items.map((item, idx) => (
          <div key={String(item.id)} className="bg-slate-50 rounded-[13px] p-[12px_14px] flex items-center gap-[10px] mb-2">
        ...
        <button className="bg-transparent border-2 border-dashed border-green-200 rounded-[13px] p-[11px] text-emerald-600 text-[14px] font-semibold cursor-pointer text-center w-full mb-1 active:bg-green-50" onClick={add}>
        ...
        <div className="bg-green-50 rounded-[13px] p-[12px_14px] mt-2">
          <div className="flex justify-between items-center">
            ...
            <span className="text-[18px] font-extrabold text-emerald-600">{fmt(total)}</span>
          </div>
        </div>
