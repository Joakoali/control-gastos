...
export default function HouseholdSetup({ user, onCreate, onJoin }: Props) {
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-indigo-600 to-violet-600 px-6 pt-[60px] flex flex-col">
      ...
      <h1 className="text-white text-[24px] font-extrabold mb-[6px]">
      ...
      {mode === 'create' && (
        <div className={card}>
          <h2 className="text-white text-[18px] font-bold mb-[10px]">Crear hogar nuevo</h2>
          ...
          <button className={btnPrimary} onClick={handleCreate} disabled={loading}>
            {loading ? 'Creando...' : '✅ Crear hogar'}
          </button>
          <button className={btnGhost} onClick={() => setMode(null)}>Volver</button>
        </div>
      )}

      {mode === 'join' && (
        <div className={card}>
          <h2 className="text-white text-[18px] font-bold mb-[10px]">Unirme a un hogar</h2>
          ...
          <input
            ...
            maxLength={6}
            className="w-full py-[14px] px-4 rounded-[14px] border-2 border-white/30 bg-white/15 text-white text-[22px] font-extrabold tracking-[6px] text-center outline-none mb-4 placeholder:text-white/40"
          />
          <button className={btnPrimary} onClick={handleJoin} disabled={loading}>
      ...
      {error && <p className="text-red-300 mt-4 text-[14px] text-center">{error}</p>}
    </div>
  )
}
